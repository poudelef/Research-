...
* 3D and Beyond Summarization: These methods are designed for data with three or more dimensions, such as meshes, skeleton points, or images with depth information (RGB+D).
   ...
   3. Spatio-temporal: This combines both spatial and temporal dimensions to summarize information, often by analyzing the motion trajectories of objects. For example, a method might estimate key-frames of a dance performance by exploiting the dancer's motion trajectories.
      ...
      * Computational cost; Correlation computations across multiple dimensions (spatio-temporal + depth) are heavy.
...
Several metrics used for summarization methods:
...
**Dataset:** SumMe, TVSum, OVP, YouTube, Kodak home video, lecture datasets, sports datasets, etc 
...
The paper compared several deep learning methods:

- **AC-SUM-GAN & VASNet** – Achieve the **best F-scores**, capturing main events while also preserving diverse content.  
- **SUM-GANdpp & Cycle-SUM** – Achieve the **highest compression ratios**, producing the shortest summaries, but may miss some context.  
- **Cycle-SUM** – Gets the **highest MOS**, as viewers find its summaries diverse and informative, especially when short textual descriptions are included.  
- **LSTM-based methods** – Handle **temporal continuity** well but fail on very slow-motion or static scenes.  
- **GAN-based methods** – Improve **diversity** by forcing summaries to resemble “realistic mini-videos.”  

**Key Trade-off:** Some methods prioritize **compactness (CR)**, while others emphasize **human perception (MOS)**.
...
Further research is needed(Table 15):
